...
- **Compiler**: needs an `ae` whose stdlib HAS the three modules — aether PR #1574. Until that merges + releases, released toolchains (≤0.537.0 at the time of writing) do NOT have them: `import std.spec` is silently tolerated but `spec.*` calls fail as undefined. Use the branch-built toolchain at `/home/paul/scm/aether/build/ae` (0.537.0 + branch `claude/integrate-aeocha-assertions-taq64v`). Once released, pin the version in README (there's a marked placeholder).
